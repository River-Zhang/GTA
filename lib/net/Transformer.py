...
class ViTDecoder(nn.Module):
    def __init__(self, image_size: Union[Tuple[int, int], int], patch_size: Union[Tuple[int, int], int],
        ...
        self.to_origin_pixiel = nn.Sequential(
            Rearrange('b (h w) c -> b c h w', h=image_height // patch_height)
        )
...
class CrossAttDecoder(nn.Module):
    def __init__(self, image_size: Union[Tuple[int, int], int], patch_size: Union[Tuple[int, int], int],
                 dim: int, depth: int, heads: int, mlp_dim: int, channels: int = 64, dim_head: int = 64) -> None:
        super().__init__()
        image_height, image_width = image_size if isinstance(image_size, tuple) \
                                    else (image_size, image_size)
        patch_height, patch_width = patch_size if isinstance(patch_size, tuple) \
...
class ViTVQ(pl.LightningModule):
    def __init__(self,image_size=512, patch_size=16,channels=3) -> None:
        ...
        self.xy_decoder = ViTDecoder(image_size=image_size, patch_size=patch_size, dim=256,depth=4,heads=8,mlp_dim=2048)
        self.yz_decoder= CrossAttDecoder(image_size=image_size, patch_size=patch_size, dim=256,depth=4,heads=8,mlp_dim=2048)
        self.xz_decoder= CrossAttDecoder(image_size=image_size, patch_size=patch_size, dim=256,depth=4,heads=8,mlp_dim=2048)
    ...
    def decode(self, enc_out: torch.FloatTensor) -> torch.FloatTensor:
        # quant = self.post_quant(quant)
        dec_xy = self.xy_decoder(enc_out)
        dec_yz = self.yz_decoder(enc_out)
        dec_xz = self.xz_decoder(enc_out)

        xy_seq=self.xy_decoder.seq
        yz_seq=self.yz_decoder.seq
        xz_seq=self.xz_decoder.seq
        
        return (dec_xy,dec_yz,dec_xz),(xy_seq,yz_seq,xz_seq)
